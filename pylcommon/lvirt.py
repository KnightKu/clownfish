...
IPV6INIT=no
"""
        # NetworkManager is the default network service on CentOS8 series
        # Need to let NM to control network
...
def gen_kickstart_script(log, bus_type, distro, internet,
                         hostname, network_configs, dns):
    """
    Generate a proper ks script for target distro
    """
...
"""
    # only disable NetworkManager for rhel6 and rhel7
    if distro in [ssh_host.DISTRO_RHEL6, ssh_host.DISTRO_RHEL7]:
        ks_config += "services --disabled=NetworkManager\n"
    ks_config += """bootloader --location=mbr --driveorder=%s --append="crashkernel=auto net.ifnames=0 biosdevname=0"\n""" % pri_disk
    ks_config += """
zerombr
clearpart --all --initlabel
"""
    ks_config += "part / --fstype=ext4 --grow --size=500 --ondisk=%s --asprimary\n" % pri_disk

    # rhel8 series need "AppStream" repo to install
    if distro == ssh_host.DISTRO_RHEL8:
        ks_config += 'repo --name="AppStream" --baseurl=file:///run/install/repo/AppStream\n'
    elif distro in [ssh_host.DISTRO_RHEL6, ssh_host.DISTRO_RHEL7]:
        ks_config += 'repo --name="Media" --baseurl=file:///mnt/source --cost=100\n'
    else:
        log.cl_error("wrong distro [%s]", distro)
        return None

    for eth_number, network_config in enumerate(network_configs):
        if internet:
            nameserver_pattern = '--nameserver=%s ' % dns
        else:
            nameserver_pattern = ""

        if "gateway" in network_config:
            gateway_pattern = '--gateway=%s ' % network_config["gateway"]
        else:
            gateway_pattern = ""

        ks_config += "network --device=ens%d --hostname=%s --bootproto=static --ip=%s --netmask=%s %s %s" \
                     "--noipv6 --onboot=yes\n" %\
                     (3 + eth_number, hostname, network_config["ip"], network_config["netmask"], gateway_pattern, nameserver_pattern)
    ks_config += """
%packages
@Core
rsync
...
"""
    for eth_index in range(len(network_configs)):
        ens_index = eth_index + 3
        ks_config += 'sed -i "s/ens%d/eth%d/g" /etc/sysconfig/network-scripts/ifcfg-ens%d\n' % \
                     (ens_index, eth_index, ens_index)
        ks_config += 'mv /etc/sysconfig/network-scripts/ifcfg-ens%d /etc/sysconfig/network-scripts/ifcfg-eth%d\n' % \
                     (ens_index, eth_index)
...
def _vm_install(log, workspace, bus_type, image_dir, ram_size, disk_sizes,
                distro, hostname, network_configs, internet, dns, server_host,
                iso_path, mnt_path):
    """
    ...
    ks_config = gen_kickstart_script(log, bus_type, distro, internet,
                                     hostname, network_configs, dns)
    if ks_config is None:
        log.cl_error("failed to generate kickstart script")
        return -1
    ...
    if distro == ssh_host.DISTRO_RHEL6:
        install_timeout = 600
    elif distro in [ssh_host.DISTRO_RHEL7, ssh_host.DISTRO_RHEL8]:
        install_timeout = 1200

    log.cl_debug("install vm [%s] via command:[%s]", hostname, command)
    log.cl_debug("kickstart script [%s]:\n%s\n", ks_fpath, ks_config)
...
def vm_install(log, workspace, server_host, iso_path, hostname, internet, dns,
    ...
    # enable "PowerTools" repo for rhel8, just reporting error without exit
    if distro == ssh_host.DISTRO_RHEL8:
        command = "yum-config-manager --enable PowerTools"
        retval = vm_host.sh_run(log, command)
        if retval.cr_exit_status:
            log.cl_error("failed to enable [PowerTools] repo on host [%s]",
                         vm_host.sh_hostname)
